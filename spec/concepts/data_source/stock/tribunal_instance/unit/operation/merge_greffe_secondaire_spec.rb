...
describe DataSource::Stock::TribunalInstance::Unit::Operation::MergeGreffeSecondaire do
  ...
  context 'success' do
    let(:entreprise_secondaire) { create :titmc_entreprise, siren: siren, code_greffe: code_greffe }
    ...
    it 'has établissement from greffe 0000' do
      etab = subject[:entreprise_secondaire].etablissements.find { |e| e.siret == '12345678900000' }
    ...
    it 'has representants from greffe 0000' do
      rep = subject[:entreprise_secondaire].representants.find { |r| r.date_creation == '1900-01-01' }
      expect(rep).to have_attributes qualite: '1200'
      expect(rep).to have_attributes code_greffe: '9712'
    end

    it 'has actes from greffe 0000' do
      acte = subject[:entreprise_secondaire].actes.find { |a| a.numero_depot_manuel == '666' }
    ...
    it 'has bilans from greffe 0000' do
      bilan = subject[:entreprise_secondaire].bilans.find { |b| b.numero == '42' }
    ...
    it 'has observation from greffe 0000' do
      obs = subject[:entreprise_secondaire].observations.find { |o| o.numero == '1234' }
