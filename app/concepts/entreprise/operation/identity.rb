module Entreprise
  module Operation
    class Identity < Trailblazer::Operation
      step Subprocess(TribunalCommerce::DailyUpdate::Operation::DBCurrentDate), Output(:fail_fast) => Track(:failure)
        fail :empty_database, fail_fast: true
      step :verify_siren
        fail :invalid_siren, fail_fast: true
      step Subprocess(DossierEntreprise::Operation::FetchImmatriculationPrincipale), Output(:fail_fast) => Track(:failure)
